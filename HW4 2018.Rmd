...
`paste0` automatically strips the spaces from between the concatenated segments. `paste` defaults to a space to separate the 
values, but allows the user to specify a different separator. 
...
** The instructions say to merge the 3 strings, but does not specify which of the 4 strings to merge so I'm just concatenating all of them. **

####Answer to Problem 2.

```{r Answer 2}

cat4 <- paste(W, Y, X, Z, collapse = NULL)
cat4
...
LengthM <- str_length(herName)
Last6 <- str_sub(herName, LengthM-5, LengthM)
Last6
...
nurses[stri_duplicated(mathematicians)]
...
stri_replace_all_fixed(pun, c(" his "," he "), c(" her "," she "), vectorize_all = FALSE)
